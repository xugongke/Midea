...
#define WDG_ENABLE  1
#define SAMPLE_NUMS  10  /**/
u8 SecMos2; /*MOS2测试时间*/



volatile unsigned char CommTimeout; //总线通讯超时计数
u8 MCtrl;       /*主显示板控制:  bit[0]:1:交流加热; 0:交流不加热   bit[7]:自检模式 1:进入自检模式    
                bit[1]:自检模式下控制直流加热  bit[1~7]暂时不使用*/
u8 CurState;    /*反馈给主显示板当前状态  
                bit[0]:1:交流加热; 0:交流不加热;
                bit[1]:直流加热状态
                bit[2]:交流电是否存在
                bit[3]:直流电是否存在
                bit[4]:直流继电器闭合控制异常(启动加热) 关闭异常   改为光伏电压过高报警   120V
                bit[5]:直流继电器断开控制异常(停止加热) 关闭异常   改为通讯异常报警 
                bit[6]:1:光伏存在,但是直流DCDC输出12v电压不存在  干烧错误 发生后, 需要断电才能恢复 
                bit[7]: 复用做光伏电源正负接反指示
                */
s8 ACTempHigh=60; /*交流温度阈值上限*/
u8 mos2_start; //第2路MOS是否启动加热  1:启动  0:未启动
u8 VolStart;    /*启动电压 判断光伏电压是否存在的阈值电压*/
u8 VolThres;    /*光伏板电压阈值 低于该电压继电器控制可能异常 高于该值才启动控制*/
...
s8 TempLow = 72;  /*温度低阈值, 低于该阈值时启动加热*/
...
void TIM1_Init()
{
    TIM1_TimeBaseInit(16-1, TIM1_COUNTERMODE_UP,    //定时器2预分频数为 16 分频，即定时器时钟 = 系统时钟/16 = 1MHz
        10000-1 ,       //设置10毫秒时间自动重载 1000
        0
        );
    TIM1_ITConfig(TIM1_IT_UPDATE , ENABLE); //使能定时器2更新中断
    TIM1_ARRPreloadConfig(ENABLE);          //允许自动重载
    TIM1_Cmd(ENABLE);        //启动定时器2开始计数
}
...
//     Get_ADC_AnyChannal_Value(&adc_value, SAMPLE_NUMS);
    ...
    if ((InputVol2+10) < InputVol){
        DCErrcnt++;
        if (DCErrcnt > 4){
           CurState |= GF_VOL_REV;//电源电压输入过低，则可能电源接反
        }
    }
    else {
        DCErrcnt = 0;
    ...
    if (InputVol < VolStart){
        DCErrcnt = 0;
        if ((CurState & DC_VOL_EXIST) != 0){ //之前有光伏电存在,则滞回清0
            if (InputVol < (VolStart-1))
            {
                /*电压值过低, 直流电不存在*/
                CurState &= ~DC_VOL_EXIST;
                CurState &= ~GF_HEAT_ON; //直流加热不存在
            }
        }
        else {
            /*电压值过低, 直流电不存在*/
    ...
    if ((CurState & DC_VOL_EXIST) != 0){ // 直流电压
        if ((GetRelayState() == STATE_CLOSE) /*&& (Temperature < TempHigh)*/){
            CurState |= GF_HEAT_ON;//直流加热
        }
        else {
            CurState &= ~GF_HEAT_ON;
        }
...
{
#if 1
    if ((GetRelayState() == STATE_CLOSE) || ((CurState & DC_VOL_EXIST) == 0) 
        || (RELAY_CTRL_STATE() == RELAY_CLOSE_CTRL_LEVEL)
        || ((CurState&GF_VOL_HIGH) != 0) //光伏电压过高
        ) {/*继电器闭合状态*/
        DEBUG_LED1_ON();
...
#else 
    /*adc检测为闭合状态，有可能是没有接加热管造成的误判*/
    if (((GetRelayState() == STATE_CLOSE) && (RELAY_CTRL_STATE() == RELAY_CLOSE_CTRL_LEVEL)) || ((CurState & DC_VOL_EXIST) == 0) 
        || ((CurState&GF_VOL_HIGH) != 0) //光伏电压过高
        ) {/*继电器闭合状态*/
            DEBUG_LED1_ON();
            // CurState &= ~(0x10);//启动加热成功
...
u8 StopHeaterRaw(void)
{
    ...
    mDelaymS(200*1);    /*继电器断开大约需要100ms @todo 实际产品中再增加一倍时间， 防止产品老化后反应时间不足*/
    MOS_CTRL1_STOP();   /*关闭MOS管*/
    //MOS_CTRL2_STOP();
    RELAY_IDLE();
    return 1;
}
...
u8 StopHeater(void)
{
    ...
    if ((GetRelayState() == STATE_DISCONNECT) || ((CurState & DC_VOL_EXIST) == 0) || (GF_12V_DECTECT_PIN == 0)) {/*继电器断开状态*/
        DEBUG_LED1_OFF();
    ...
    if (GetRelayState() == STATE_DISCONNECT) {/*继电器断开状态*/
        ...
        /*停止加热报警*/
        if (((CurState & DC_VOL_EXIST) == 0) || (GF_12V_DECTECT_PIN == 0)){ 
            //CurState &= ~(0x20);//停止加热成功
            return 1; /*操作成功*/ 
        }
        else {
...
void DelaySecond(u16 sec)
{
    ...
    while (sec--)
    {
        ...
        if (InputVol >= 120){
            VolErrCnt++;
            if (VolErrCnt > 3){
                CurState |= GF_VOL_HIGH; //光伏电压过高
            }
        }
        else {
            VolErrCnt=0;
            CurState &= ~GF_VOL_HIGH;
        }
...
u32 start_tick; 
u32 state_tick;
void NormalDelayProc(void)
{
    if ((tick_10ms - state_tick) >= (ONE_SECOND_TICK/2)){
        GetACDectect();
        state_tick = tick_10ms;
        InputVol = GetInputVol();
        GetTemperature();
        if (CommTimeout < 40) {
            CommTimeout++;
...
u8  SysError; /*是否发生系统错误*/
void ErrStateProc(void)
{
    if (CommTimeout >= 31){
        CurState |= COM_ERR_ST;
    }

    if ((CurState & (GF_VOL_HIGH | BURNED_PROTECT | GF_VOL_REV | COM_ERR_ST)) != 0){
        /*光伏电压过高 干烧 电源接反 通讯错误 等错误*/
        SysError = 1;
    }

    if ((Temperature < ERR_TEMP) || (Temperature >= 92)){
        SysError = 1;//温度超出正常允许范围
    }
...
//   ReliabilityTest();
 ...
 // CloseHeater();
    ...
    while (1)
    {
      ...
      //  GetTemperature();
        ErrStateProc();
        if ((MCtrl & 0x80) != 0){ /*自检模式*/
            if (((MCtrl & 0x01) != 0) && ((CurState & AC_POWER_ON) != 0)){
                /*交流加热*/
                /*启动交流加热*/
                CurState |= AC_HEAT_ON;
                AC_RELAY_ON();
            ...
            if (((MCtrl & 0x02) != 0) && ((CurState & DC_VOL_EXIST) != 0)){
                StartHeater();
            }
            else {
                StopHeater();
           ...
           /*通讯超时判断*/
           if ((tick_10ms - start_tick) >= (ONE_SECOND_TICK/2)){
               CommTimeout++;
               start_tick = tick_10ms;
               InputVol = GetInputVol();
               GetACDectect();
               GetTemperature();
           }
        ...
        /*市电不存在,且光伏电压低则需考虑保护机制*/
//         if ((InputVol < VolThres) && (CurState&0x04 == 0)) {
//             /*当前电压过低不进行控制*/
//             DelaySecond(5);
//             InputVol = GetInputVol();
