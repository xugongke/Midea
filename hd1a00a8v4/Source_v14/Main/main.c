...
void ReliabilityDemoTest(void)
{
  ...
  //  StateCnt = 0;
    ...
    case 0:
        ...
        ACHeatFlag  = 1;
        JLFlag      = 1;
        if ((time_boot-t_s_rel) >= (10)) /*10秒 显示交流加热*/{
            t_s_rel = time_boot;
            StateCnt = 8;
        }
    ...
    case 8: /*开始显示直流加热图标*/
        ...
        ACHeatFlag  = 0;
        JLFlag      = 0;
        if ((time_boot-t_s_rel)  >= (10)) /*10秒 显示直流加热*/{
            t_s_rel = time_boot;
            StateCnt = 0;
        }
        break;
    ...
    if ((tick - send_start_tick) >= TICKS_ONE_SECOND/30){
        RecvDataProc();
        uart0_send(SendDataCons());/*串口同步控制板*/
    }
}
#endif
#if TEST_MODE_EN
...
//             }
            ...
            if ((GFFlag != 0) || (JLFlag != 0)){
                ...
                case 2: //显示升数 60L
                    ...
                    if ((tick -powerup_tick) > (TICKS_ONE_SECOND/3)){
                        /*0.5秒升数显示结束*/
                        PowerStateCnt = 3;
                        powerup_tick = tick;
                    }
...
void CheckModeProc(void)
{
    ...
    case 0:
        ...
        break;
    case 1:
        if ((tick-StateStartTick) > (/*TICKS_ONE_SECOND+*/TICKS_ONE_SECOND/2+TICKS_ONE_SECOND/5)) /*1.5秒交流加热结束*/{
            StateCnt = 2;
            StateStartTick = tick;
        }
        break;
    case 2:
        if ((tick-StateStartTick) > (TICKS_ONE_SECOND/2+TICKS_ONE_SECOND/5)) /*1秒停止加热*/{
            StateCnt = 3;//开始直流加热
            StateStartTick = tick;
        }
        break;
    case 3:
        if ((tick-StateStartTick) > (/*TICKS_ONE_SECOND+*/TICKS_ONE_SECOND/2+TICKS_ONE_SECOND/5)) /*1.5秒直流加热结束*/{
            StateCnt = 0;
            StateStartTick = tick;
            FactoryLEDOff = 0; //以后屏幕显示正常 工厂模式下不再熄屏
            ModeExchange(); /*@TODO 处理记录进入自检模式前状态*/
        }
        break;
    default:
...
{ 
    ...
    case FSM_IDLE: //待机状态
        ...
        if (FactorMode != 0){
            if ((key_valid == KEY_VALUE_DOWN) || 
                (key_valid == KEY_VALUE_UP)){
                    key_valid = 0;
                    if (time_boot <= 6){ //关机状态下也可以响应功率自检模式
                        StateCnt = 0;
                        StateStartTick = tick;
...
//             }
    ...
    /*故障检查  传感器短路和断路故障*/
    if ((TempCur <= ERR_TEMP) || (TempCur > 119)){
        RunState = FSM_ERR;
        RunStateFlag |= FAULT_FLAG;
        HEAT_OFF();
        ErrCode = 4;
