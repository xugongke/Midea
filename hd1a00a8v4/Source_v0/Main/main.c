    ...
    case FSM_OPEN:
        ...
        if (time_sec > 3){
            ...
            if (SetingIndex == 1){
                SetingIndex = 0; //温度设置完
                updown_flag = 0;
                if (EEPROM_ByteRead(0) != TempHeatUp){  /*存储设置温度*/
                    EEPROM_ByteWrite(0, TempHeatUp);
                }
                break;
            }
...
void BEEP_PROC(){
    ...
    //cur_key |= ((DATA_BUF[1] >> 6)&0x03) ; //key 15  KEY16
    //cur_key |= ((DATA_BUF[2]&0X0F) << 2); //key 17~key20
    //cur_key &= KEY_ALL;
    /*LED灯显示处理*/
    ...
    /*蜂鸣器*/
    // if ((key_valid != 0) || (State == FSM_ERR)) {
    //  time_sleep = time_boot;
    //       i = 30;
    //         do 
    //         {
    //             BEEP_LED = 0;
